...
from data.users import problem_user
...
@allure.title(f'Авторизация {problem_user.login}')
def test_authorization_pu(page: Page, pobj: AllPages):
    with allure.step('Предусловие: Перейти на страницу авторизации'):
        pobj.login.navigate()
    with allure.step('1 Заполнить поле "Логин"'):
        pobj.login.input_username.fill(problem_user.login)
    with allure.step('2 Заполнить поле "Пароль"'):
        pobj.login.input_password.fill(problem_user.password)
