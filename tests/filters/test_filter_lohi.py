...
@allure.title('Фильтры LOHI')
def test_filters_lohi(page: Page, pobj: AllPages):
    with allure.step('Предусловие: Аторизация, открыта Главная'):
        pobj.login.do_login()
    with allure.step('1 выбрать сортировку LOHI'):
        pobj.main.select_filter_option('lohi')
    ...
    with allure.step('3 порядок элементов совпадает с ожидаемым'):
        sorted_inventory_list = sorted(copy(inventory_list), key=lambda e: e.price)
